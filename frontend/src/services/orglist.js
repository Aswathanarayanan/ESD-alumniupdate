...
const orglistUrl = `http://localhost:8080/esd-project-1.0-SNAPSHOT/api/organisation/getorg`

const orglist = async () => {
  // Send the login credential data to the loginBaseUrl API endpoint as an HTTP POST request
  // Note the async-await
  const response = await axios.get(orglistUrl)
